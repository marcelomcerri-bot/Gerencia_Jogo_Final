...
- **Modo Professor**: New spectator dashboard accessible from the home menu. Professor sees a live grid of all students' game states (room, prestige, energy, stress, missions, last activity) updated every 3s. Students are registered automatically when starting a new game. The API is embedded directly in the Vite dev server via a custom `roomApiPlugin()` in `vite.config.ts` (no second server needed). Room state is in-memory; players inactive >90s are cleaned up automatically. **IMPORTANT**: API path is `/__rooms/` (NOT `/api/rooms/`) — Replit's proxy intercepts `/api/*` with 502 errors. Netlify deployment uses `netlify/functions/rooms.mjs` with `node_bundler = "none"` to avoid bundling hang.
